...
use crate::keys::{DerivableKey, DescriptorSecretKey, ExtendedKey};
...
impl<K: DerivableKey<Legacy>> DescriptorTemplate for Bip47Notification<K> {
    fn build(self,network:Network) -> Result<DescriptorTemplateOut, DescriptorError> {
        ...
        .build(network) //an argument of type `bitcoin::Network` was missing
    }
}
...
impl<'w, D: BatchDatabase> Bip47Wallet<'w, D> {
    ...
    fn derive_inbound_wallet(
        ...
        pk.mul_tweak(secp, &scalar::Scalar::from(sk))?; //Use mul_tweak instead
    ...
    fn generate_blinding_factor(
        ...
        let shared_secret = SharedSecret::new(
            ...
            &keys_map.values().next().expect("Key is present"),
        );
